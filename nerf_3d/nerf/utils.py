...
class Trainer(object):
    ...
    def train_step(self, data):
        ...
        image_z_1 = self.clip_model.encode_image(self.aug(ori_img))
        image_z_2 = self.clip_model.encode_image(self.aug(resized_tensor))
        ...
        if self.opt.lambda_smooth > 0 and 'loss_smooth' in outputs:
            loss_smooth = outputs['loss_smooth'] #表面平滑性
    ...
    def test_step(self, data, bg_color=None, perturb=False):  
        ...
        outputs = self.model.render(rays_o, rays_d, depth_scale=depth_scale, staged=True, perturb=perturb, light_d=light_d,
    ...
    def test(self, loader, save_path=None, name=None, write_image=True, write_video=True):
        ...
        with torch.no_grad():
            for i, data in enumerate(loader):
                ...
                phis.append(data['phis'].item())
                degrees = math.degrees(data['phis'].item())
                jiaodu.append(degrees)
                # print(preds_mask)
    ...
    def train_one_epoch(self, loader):
        ...
        # jiaodu = []
        # for data in loader:
        #     thetas.append(data['thetas'].item())
        #     degrees = math.degrees(data['thetas'].item())
        #     jiaodu.append(degrees)
        #     self.log(f"==> Finished Epoch {self.epoch}.")
        #     # update grid every 16 steps
        #     if self.model.cuda_ray and self.global_step % self.opt.update_extra_interval == 0:
