...
from GAN import modelG, conv_liner
...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        self.image_segmentation_total = None
        self.shared_x = None
        self.new_channel = torch.from_numpy(np.zeros((1, 64, 64))).to(device)
        self.x = None
        if self.use_ema:
    ...
    def get_loss(self, pred, target, mean=True):
        image_segmentation_total = self.image_segmentation_total
        shape_x = self.shared_x
        x = self.x
        ...
        if self.loss_type == 'l1':
            ...
            if mean:
                ...
                #     loss = loss1 + loss
                    ...
                    #         loss1 = loss1.item()
                    #         loss2 = sublist.item()
                    #         new_tuple = torch.tensor(loss2 + loss1).to(device)
                    #         loss_new = (new_tuple, loss[1])
                    #         loss = loss_new
        else:
    ...
    def generate(self, data, num_samples, ddim_steps=300, HW=None, limit=None, state=None):
        ...
        with model.ema_scope():
            for count, item in enumerate(zip(data['eeg'], data['label'], data['image'], data['fre_eeg'], data['time_eeg'])):
                if limit is not None:
                    if count >= limit:
                        break
                latent = item[0] # eeg embedding
                target = item[1]
                gt_image = rearrange(item[2], 'h w c -> 1 c h w') # h w c
                fre_eeg = item[3]
                time_eeg = item[4]
                ...
                # # Further optimization with enhanced data
                # if (item[0].shape[0] == batch_size_eeg):
                #     latent = item[0]
                #     target = item[1]
                #     # SAGAN扩充
                #     output_conv_liner = conv_liner(latent)
                #     fake = modelG(output_conv_liner).view(4, 128, 512)
                #     gt_image = rearrange(item[2], 'h w c -> 1 c h w')  # h w c
                #     print(f"rendering {num_samples} fake examples in {ddim_steps} steps.")
                #     c, re_latent = model.get_learned_conditioning(repeat(fake, 'h w -> c h w', c=num_samples).to(self.device), target, fre_eeg, time_eeg)
                #     samples_ddim, _ = sampler.sample(S=ddim_steps,
                #                                      conditioning=c,
                #                                      batch_size=num_samples,
                #                                      shape=shape,
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.validation_count % 1 == 0 and self.validation_count != 0 :
...
class LatentDiffusion(DDPM):
    ...
    def get_learned_conditioning(self, c, target, fre_eeg, time_eeg):
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        # self.cond_stage_model.eval()
        if hasattr(self.cond_stage_model, 'encode') and callable(self.cond_stage_model.encode):
            c, re_latent = self.cond_stage_model.encode(c,target, fre_eeg, time_eeg)
            # c = self.cond_stage_model.encode(c)
        else:
            c, re_latent= self.cond_stage_model(c,target, fre_eeg, time_eeg)
    ...
    def shared_step(self, batch, **kwargs):
        ...
        self.x = x
        shape_x = x.shape[0]
        self.shared_x = shape_x

        fre_eeg = batch['fre_eeg']
        time_eeg = batch['time_eeg']
        # original_image = batch['image']
        # original_image = np.squeeze(original_image[0:1,:,:].cpu().numpy())
        # original_image = cv2.resize(original_image, (64, 64))

        image_segmentation_total = batch['image_segmentation']
        self.image_segmentation_total = image_segmentation_total
        ...
        if self.return_cond:
            ...
            # 这次debug关于x的维度 torch.Size([4, 4, 64, 64])
            loss = self(x, c, label, image_raw, fre_eeg, time_eeg)
            ...
            #     image_segmentation = torch.from_numpy(image_segmentation).to(device)
            #     img = torch.from_numpy(img).to(device)
            #     loss1 = torch.nn.functional.mse_loss(image_segmentation, img)
            #     for sublist in loss:
            #         if type(sublist)!= dict:
            #             loss1 = loss1.item()
            #             loss2 = sublist.item()
            ...
            # image_with_mask = image_with_mask.permute(2, 0, 1)
            #在x这继续loss值
            # loss = torch.nn.functional.mse_loss(target, pred)
    ...
    def forward(self, x, c, label, image_raw, fre_eeg, time_eeg, *args, **kwargs):
        ...
        if self.model.conditioning_key is not None:
            ...
            if self.cond_stage_trainable:
                ...
                c, re_latent = self.get_learned_conditioning(c, label, fre_eeg, time_eeg)
                # print('c.shape')
                # print(c.shape)
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        loss_simple = self.get_loss(model_output, target, mean=False).mean([0, 1, 2])
        loss_dict.update({f'{prefix}/loss_simple': loss_simple.mean()})

        logvar_t = self.logvar[t].to(self.device)
        
#         loss = loss_simple / torch.exp(logvar_t.view(64)) + logvar_t.view(64)
        #back
        # print(logvar_t.shape)
        if(logvar_t.shape!=4):
            logvar_t = logvar_t.repeat(16)
        else:
            print(logvar_t)
            logvar_t = logvar_t.repeat(64)
            print(logvar_t)
            print(loss_simple)
...
class EEGClassifier(pl.LightningModule):
    ...
    def full_validation(self, batch, state=None):
        ...
        print(acc1, acc5)
        # acc1, acc5 = accuracy(imageNet_images, labels, topk=(1, 5))
        # grid_imgs = Image.fromarray(grid.astype(np.uint8))
