...
class cond_stage_model(nn.Module):
    def __init__(self, metafile, num_voxels=440, cond_dim=1280, global_pool=True, clip_tune = True, cls_tune = False):
        ...
        self.blip_model = Blip2ForConditionalGeneration.from_pretrained("Salesforce/blip2-opt-2.7b", torch_dtype=torch.float16).to("cuda")
    ...
    def forward(self, x):
        ...
        # n, c, w = x.shape
        inputs = self.processor(image_pil, return_tensors="pt").to("cuda", torch.float16)
...
class eLDM:
    ...
    def __init__(self, metafile, num_voxels, device=torch.device('cpu'),
        ...
        config = OmegaConf.load('/root/autodl-tmp/DreamDiffusion/pretrains/models/config15.yaml')
        ...
        pl_sd = torch.load('/root/autodl-tmp/DreamDiffusion/v1-5-pruned.ckpt', map_location="cpu")['state_dict']
