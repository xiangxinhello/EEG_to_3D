...
class eeg_pretrain_dataset(Dataset):
    def __init__(self, path='../dreamdiffusion/datasets/mne_data/', roi='VC', patch_size=16, transform=identity, aug_times=2, 
        ...
        data = []
        images = []
        self.input_paths_eeg2 = '/root/autodl-tmp/sub-01/preprocessed_eeg_training.npy'
    ...
    def __getitem__(self, index):
        ...
        # data_path_eeg2 = self.input_paths_eeg2
        # data_eeg2 = np.load(data_path_eeg2, allow_pickle=True).item()
...
class EEGDataset(Dataset):
    ...
    def __init__(self, eeg_signals_path, image_transform=identity, subject = 4):
        # Load EEG signals
        eeg_signals_path = '/root/autodl-tmp/DreamDiffusion/datasets/mne_data/eeg_5_95_std.pth'
        loaded = torch.load(eeg_signals_path)
        # if opt.subject!=0:
        #     self.data = [loaded['dataset'][i] for i in range(len(loaded['dataset']) ) if loaded['dataset'][i]['subject']==opt.subject]
        # else:
        ...
        self.labels = loaded["labels"]
        self.images = loaded["images"]
        self.imagenet = '/root/autodl-tmp/DreamDiffusion/image_new_thing/'
        self.imagenet_segmentation = '/root/autodl-tmp/DreamDiffusion/imageNet_images_new_segmentation/'
        self.image_transform = image_transform
        self.num_voxels = 440
        self.data_len = 512
        # Compute size
    ...
    def __getitem__(self, i):
        ...
        target = self.data[i]["label"]

        # Get label
        image_name = self.images[self.data[i]["image"]]
        image_path = os.path.join(self.imagenet, image_name.split('_')[0], image_name+'.JPEG')

        image_name_segmentation = self.images[self.data[i]["image"]]
        image_path_segmentation = os.path.join(self.imagenet_segmentation, image_name_segmentation.split('_')[0], image_name_segmentation+'.JPEG')
        ...
        image_raw_segmentation = Image.open(image_path_segmentation).convert('RGB')
        image_segmentation = np.array(image_raw_segmentation) / 255.0


        return {'eeg': eeg, 'label': label, 'image': self.image_transform(image),
                'image_raw': image_raw, 'fre_eeg': fre_eeg, 'time_eeg':time_eeg,
                'image_segmentation': self.image_transform(image_segmentation)
                }
...
class Splitter:
    ...
    def __init__(self, dataset, split_path, split_num=0, split_name="train", subject=4):
        ...
        # Load split
        split_path = '/root/autodl-tmp/DreamDiffusion/datasets/mne_data/block_splits_by_image_single.pth'
...
def create_EEG_dataset(eeg_signals_path='../dreamdiffusion/datasets/eeg_5_95_std.pth', 
            ...
            image_transform=identity, subject =4):
    # if subject == 0:
        # splits_path = '../dreamdiffusion/datasets/block_splits_by_image_all.pth'
